<template>
  <div class="company_detail">
    ...
    <el-tabs
      ...
      @tab-click="changeTab"
      v-if="companyAuth && companyAuth.child.length > 0"
    >
      <el-tab-pane
        label="基础信息"
        name="基础信息"
        v-if="isShowTab(companyAuth.child, '基础信息')"
      >
        <div style="position: relative" v-show="currentIndex == '基础信息'">
          <BaseForm :type="type" :formData="{ ...baseInfo }" ref="baseComp" />
          <div>
            <el-button type="primary" @click="edit(0)" v-if="type == 1"
              >提交</el-button
            >
            <el-button
              type="primary"
              @click="edit(1)"
              v-if="type == 0"
              :disabled="baseInfo && baseInfo.status == 0"
            >
              编辑
            </el-button>
            <el-button type="primary" plain @click="cancelEdit" v-if="type == 1"
              >取消</el-button
            >
          </div>
        </div>
      ...
      <el-tab-pane
        label="人员列表"
        name="人员列表"
        v-if="isShowTab(companyAuth.child, '人员列表')"
      >
        <DispatchTable
          :companyId="id"
          v-show="currentIndex == '人员列表'"
          ref="dispatchTable"
        />
      </el-tab-pane>
      <el-tab-pane
        label="社保名单"
        name="社保名单"
        v-if="isShowTab(companyAuth.child, '社保名单')"
      >
      ...
      <el-tab-pane
        label="员工薪资"
        name="员工薪资"
        v-if="isShowTab(companyAuth.child, '员工薪资')"
      >
      ...
      <el-tab-pane
        label="费用管理"
        name="费用管理"
        v-if="isShowTab(companyAuth.child, '费用管理')"
      >
        <ChargeTable
          :companyId="id"
          :companyName="baseInfo && baseInfo.name"
          v-if="currentIndex == '费用管理'"
        />
      </el-tab-pane>
      <el-tab-pane
        label="补偿金"
        name="补偿金"
        v-if="isShowTab(companyAuth.child, '补偿金')"
      >
...
export default {
  ...
  created() {
    if (this.$route.query && this.$route.query.isCK) {
    ...
    this.id = id;
    this.type =
      action === "edit" ? 1 : this.pageMeta.title == "公司列表" ? 2 : 0;
    this.btns =
      this.type == 1
        ? ["基础信息"]
        : [
  ...
  watch: {
    ...
    companyAuth() {
      if (this.companyAuth && this.companyAuth.child.length > 0) {
        let result = this.companyAuth.child.map(
          (item) => item.title == "人员列表"
        );
        if (result) {
          if (this.$route.query && this.$route.query.isCK) {
            return;
          } else {
            this.currentIndex = this.$route.path.includes("/companylist")
              ? this.companyAuth.child[0].title
              : "人员列表";
            return;
          }
        }
  ...
  methods: {
    ...
    changeTab() {
      ...
      this.$router.push({
        path,
        query: { current: this.currentIndex, isCK: true },
      });
      ...
      if (this.currentIndex == "人员列表") {
        bus.$emit("dispatchPerson", 4);
      }
    },
    clickPanel(index) {
      switch (index) {
